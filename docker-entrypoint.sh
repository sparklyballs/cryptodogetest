...
cryptodoge configure --upnp "${upnp}"
...
if [[ -n "${peer_count}" ]]; then
  cryptodoge configure --set-peer-count "${peer_count}"
fi

if [[ -n "${outbound_peer_count}" ]]; then
  cryptodoge configure --set_outbound-peer-count "${outbound_peer_count}"
fi
...
sed -i 's/localhost/127.0.0.1/g' "$CRYPTODOGE_ROOT/config/config.yaml"

if [[ ${log_to_file} != 'true' ]]; then
  sed -i 's/log_stdout: false/log_stdout: true/g' "$CRYPTODOGE_ROOT/config/config.yaml"
else
  sed -i 's/log_stdout: true/log_stdout: false/g' "$CRYPTODOGE_ROOT/config/config.yaml"
fi
