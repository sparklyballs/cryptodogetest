FROM python:3.9 AS cryptodoge_build
...
RUN \
	...
	&& git clone --branch ${RELEASE} --recurse-submodules=mozilla-ca https://github.com/CryptoDoge-Network/cryptodoge.git . \
	&& /bin/sh ./install.sh
...
ENV \
        CRYPTODOGE_ROOT=/root/.cryptodoge/mainnet \
        farmer_address= \
        farmer_port= \
        keys="generate" \
        log_level="INFO" \
...
# copy build files
COPY --from=cryptodoge_build /cryptodoge-blockchain /cryptodoge-blockchain
